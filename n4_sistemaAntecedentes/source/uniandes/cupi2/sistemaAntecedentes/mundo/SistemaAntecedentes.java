...
public class SistemaAntecedentes
{
    ...
    public void agregarProcesadoYAntecedente( String pNombre, String pApellido, String pIdentificacion, String pSexo, int pEdad, int pIdentificadorAntecedente, String pTipoAntecedente, String pDescripcionAntecedente ) throws Exception
    {
    	...
    	else
    	{
    		try 
    		{
				proc = new Procesado(pNombre, pApellido, pIdentificacion, pSexo, pEdad);
				proc.agregarAntecedente(pIdentificadorAntecedente, pTipoAntecedente, pDescripcionAntecedente);
				procesados.add(proc);
			} 
    		catch (Exception e) 
			{
    			...
    			proc.agregarAntecedente(pIdentificadorAntecedente, pTipoAntecedente, pDescripcionAntecedente);
				procesados.add(proc);
				throw new Exception(e.getMessage() + "Se cambió la edad del procesado por la edad mínima permitida.");
			}
    	}
    ...
    public void agregarAntecedenteAProcesado( String pIdentificacion, int pIdentificadorAntecedente, String pTipoAntecedente, String pDescripcionAntecedente ) throws Exception
    {
    	...
    	{
    		procesados.get(buscarProcesado(pIdentificacion)).agregarAntecedente(pIdentificadorAntecedente, pTipoAntecedente, pDescripcionAntecedente);;
    	}
    ...
    /**
     * Este método busca la lista de procesados y busca los procesados que tienen un antecedente del tipo dado por parámetro.
     * <b> pre: </b> La lista de procesados está incializada. <br>
     * @param pTipoAntecedente Tipo del antecedente a buscar. pTipoAntecedente != null && pertenece a { Antecedente.HOMICIDO, Antecedente.VIOLENCIA, Antecedente.ROBO,
     *        Antecedente.ECOLOGICO, Antecedente.TRANSITO, Antecedente.SECUESTRO, Antecedente.ALLANAMIENTO_DE_MORADA, Antecedente.ESTAFA, Antecedente.CONSUMO_DE_DROGRAS,
     *        Antecedente.TRÁFICO_DE_DROGRAS }.
     * @return Devuelve una lista con todos los procesados con el tipo dado por parámetro.
     * @throws Exception En caso que no exista ningún procesado con un antecedente del tipo dado.
     */
    ...
    public int buscarProcesadoConMasAntecedentes( ) throws Exception
    {
    	...
    	{
    		int size = procesados.size();
    		int index = -1;
    		int numMax = 0;
    		int numComp = 0;
    		for(int i = 0; i < size; i++)
    		{
    			numComp = procesados.get(i).darNumeroAntecedentes();
    			if(numMax < numComp)
    			{
    				numMax = numComp;
    				index = i;
    ...
    public int buscarProcesadoConMenosAntecedentes( ) throws Exception
    {
    	...
    	else
    	{
    		int index = 0;
    		int size = procesados.size();
    		int numMin = procesados.get(0).darNumeroAntecedentes();
    		int numComp = 0;
    ...
    public int buscarProcesadoConMasAnhosCondena( ) throws Exception
    {
    	...
    	{
    		int size = procesados.size();
    		boolean todosMenor = true;
    		for(int i = 0; i < size && todosMenor; i++)
    		{
    			if(procesados.get(i).esMayorDeEdad())
    			{
    				todosMenor = false;
    			}
    		}
    		...
    		else
    		{
    			...
    			{
    				anComp = procesados.get(j).darAnhosCondena();
    				if(anMax < anComp)
    				{
    					anMax = anComp;
    					index = j;
    ...
    /**
     * Elimina al procesado cuyo número de identificacion está indicado por parámetro.
     * <b> pre: </b> La lista de procesados está inicializada. <br>
     * @param pIdentificacion Número de identificación del procesado a eliminar. pIdentificacion != null && pIdentificacion != "".
     * @throws Exception Si no se encuentra el procesado en el sistema.
     */
