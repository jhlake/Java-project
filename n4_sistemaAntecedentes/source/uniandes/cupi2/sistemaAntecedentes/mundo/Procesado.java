...
public class Procesado
{
    ...
    /**
     * Busca un antecedente dado el identificador dado por parámetro, y lo devuelve.
     * <b> pre: </b> La lista de antecedentes está inicializada. <br>
     * @param pIdentificadorAntecedente El número identificador del antecedente. pIdentificadorAntecedente >= 0.
     * @return Antecedente que corresponde al número identificador dado por parámetro.
     */
    ...
    public void condenarProcesado( int pAnhosCondena, int pIdentificadorAntecedente ) throws Exception
    {
    	...
    	else
    	{
    		...
    		{
    			actual = antecedentes.get(i);
    			if(actual.darIdentificador() == pIdentificadorAntecedente)
    			{
    				actual.cambiarCondena(pAnhosCondena);
    				cond = true;
    			}
    		}
    	}
    ...
    public void eliminarAntecedente( int pIdentificadorAntecedente ) throws Exception
    {
    	...
    	boolean elim = false;
    	int size = antecedentes.size();
    	for(int i = 0; i < size && !elim; i++)
    	{
    		actual = antecedentes.get(i);
    		if(actual.darIdentificador()== pIdentificadorAntecedente)
    		{
    			if(actual.darGravedad() != Antecedente.LEVE)
    			{
