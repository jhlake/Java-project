...
public class Zoologico
{
    ...
    public int darCantidadJaulasNorte( )
    {
        ...
        {
            jaulaActual = jaulas[i];
            if(jaulaActual.darSector().equals(Jaula.NORTE))
            {
                cantidad++;
            }
            i++;
    ...
    public int agregarAnimal( String pNombre, String pEspecie, String pTipo, String pHabitat, String pImagen )
    {
    	...
    	int sePudo = -1;
    	Jaula jaulaActual = null;
    	while(i<CANT_JAULAS && sePudo == -1)
    	{
    		jaulaActual = jaulas[i];
    		if(jaulaActual.puedeAgregarPorHabitat(pHabitat)&&jaulaActual.puedeAgregarPorTipoYEspecie(pEspecie, pTipo))
    		{
    			jaulaActual.agregarAnimal(pNombre, pEspecie, pTipo, pHabitat, pImagen);
    			sePudo = i;
    		}
    		i++;
    	}
    ...
    public int buscarJaulaAnimalPorNombre( String pNombreAnimal )
    {
    	...
    	int indexJaula = -1;
    	Jaula jaulaActual = null;
    	while(i<CANT_JAULAS && indexJaula == -1)
    	{
    		jaulaActual = jaulas[i];
    		if(jaulaActual.existeAnimalEnJaula(pNombreAnimal))
    		{
    			indexJaula = i;
    		}
    		i++;
    ...
    public int buscarJaulaMayorCantidadAnimales( )
    {
    	...
    	while(i<CANT_JAULAS && indexJaula == -1)
    	{
    		if(jaulas[i].darCantidadAnimales() > numAnMax)
    		{
    			numAnMax = jaulas[i].darCantidadAnimales();
    			indexJaula = i;
    		}
    		i++;
    	}
    ...
    public boolean existenJaulasConMismaCantidadAnimales( )
    {
    	...
    	int i = 0;
    	int count = 1;
    	boolean siHayMismaCantidad = false;
    	Jaula temp = null;
    	while(i<CANT_JAULAS && !(siHayMismaCantidad))
    	{
    		temp = jaulas[i];
    		while(count< CANT_JAULAS&&!(siHayMismaCantidad))
    		{
    			if(temp.darCantidadAnimales() == jaulas[count].darCantidadAnimales())
    			{
    				siHayMismaCantidad = true;
    			}
    			count++;
    		}
    		i++;
    	}
    	return siHayMismaCantidad;
    }
